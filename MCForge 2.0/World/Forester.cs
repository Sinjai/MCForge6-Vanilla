...
namespace MCForge.World
{
    ...
    public static class Forester
    {
        ...
        public static void Generate( ForesterArgs args)
        {
        ...
        public static void Plant( ForesterArgs args, Vector3I treeCoordinate)
        {
            List<Tree> treeList = new List<Tree> {
                new Tree {
                    Args = args,
        ...
        public static void SexyPlant( ForesterArgs args, Vector3I treeCoordinate)
        {
            List<Tree> treeList = new List<Tree> {
                new Tree {
                    Args = args,
        ...
        static void FindTrees(ForesterArgs args, ICollection<Tree> treelist)
        {
            ...
            for (int x = 0; x < args.Map.Size.Width; x++)
            {
                for (int z = 0; z < args.Map.Size.Length; z++)
                {
                    int y = args.Map.Size.Height - 1;
                    while (true)
                    {
                        int foliagetop = args.Map.SearchColumn(x, z, args.FoliageBlock, y);
                        if (foliagetop < 0) break;
                        y = foliagetop;
                        Vector3F trunktop = new Vector3F(x, y - 1, z);
        ...
        static Vector3I FindRandomTreeLocation(ForesterArgs args, int height)
        {
            ...
            int mindim = Math.Min(args.Map.Size.Width, args.Map.Size.Length);
            if (padding > mindim / 2.2)
            {
                padding = (int)(mindim / 2.2);
            }
            int x = args.Rand.Next(padding, args.Map.Size.Width - padding - 1);
            int z = args.Rand.Next(padding, args.Map.Size.Length - padding - 1);
        ...
        static void PlantMangroves(ForesterArgs args, ICollection<Tree> treelist)
        {
            ...
            while (treelist.Count < args.TreeCount && attempts < MaxTries)
            {
                ...
                int x = args.Rand.Next(padding, args.Map.Size.Width - padding - 1);
                int z = args.Rand.Next(padding, args.Map.Size.Length - padding - 1);
                int top = args.Map.Size.Height - 1;

                int y = top - DistanceToBlock(args.Map, new Vector3F(x, z, top), Vector3F.Down, Block.BlockList.AIR, true);
                int dist = DistanceToBlock(args.Map, new Vector3F(x, z, y), Vector3F.Down, Block.BlockList.WATER, true);
        ...
        static void ProcessTrees(ForesterArgs args, IList<Tree> treelist)
        {
            ...
            for (int i = 0; i < treelist.Count; i++)
            {
                ...
                if (args.MapHeightLimit)
                {
                    ...
                    int mapHeight = args.Map.Size.Height;
                    int foliageHeight;
        ...
        class ProceduralTree : Tree
        {
            ...
            void MakeRoots(IList<RootBase> rootbases)
            {
                ...
                foreach (Vector3I coord in FoliageCoords)
                {
                    ...
                    Vector3I offset = new Vector3I(startcoord - coord);

                    if (Args.Shape == TreeShape.Mangrove)
                    {
                        // offset = [int(val * 1.618 - 1.5) for val in offset]
                        Vector3F poo = new Vector3F((float)offset.X, (float)offset.Y, (float)offset.Z);
                        var preOffset = (poo * 1.618f - HalfBlock * 3);
                        offset = new Vector3I((int)preOffset.X, (int)preOffset.Z, (int)preOffset.Y);
                    }
                    ...
                    {
                        int offlength = (int)offset.Length;
                        if (offlength < 1) continue;
                        float rootmid = endsize;
                        Vector3I vec = offset / offlength;

                        Block searchIndex = Block.BlockList.AIR;
                        if (Args.Roots == RootMode.ToStone)
                        {
                            searchIndex = Block.BlockList.STONE;
                        }
                        else if (Args.Roots == RootMode.Hanging)
                        {
                            searchIndex = Block.BlockList.AIR;
                        }
                        ...
                        dist = startdist + DistanceToBlock(Args.Map, (Vector3F)searchstart, (Vector3F)vec, searchIndex);

                        if (dist < offlength)
                        {
                            rootmid += (rootstartsize - endsize) * (1 - dist / offlength);
                            endcoord = new Vector3I(startcoord + vec * (int)dist);
        ...
        static int DistanceToBlock(Level map, Vector3F coord, Vector3F vec, Block blockType)
        {
            return DistanceToBlock(map, coord, vec, blockType, false);
        }

        static Vector3F HalfBlock = new Vector3F(.5f, .5f, .5f);
        static int DistanceToBlock(Level map, Vector3F coord, Vector3F vec, Block blockType, bool invert)
        {
            coord += HalfBlock;
            int iterations = 0;
            while (map.InBounds(new Vector3I(coord)))
            {
                Block blockAtPos = map.GetBlock(new Vector3S((ushort)coord.X, (ushort)coord.Z, (ushort)coord.Y));
    ...
    public sealed class ForesterArgs
    {
        ...
        public Block PlantOn = Block.BlockList.GRASS;
        public Random Rand;
        public Level Map;

        public Block TrunkBlock = Block.BlockList.WOOD;
        public Block FoliageBlock = Block.BlockList.LEAVES;
