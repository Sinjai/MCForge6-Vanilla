...
namespace MCForge.World
{
    ...
    /// <summary> Level generator themes. A theme defines what type of blocks are used to fill the map. </summary>
    ...
    /// <summary> Level generator template. Templates define landscape shapes and features. </summary>
    ...
    public class MapGenerator
    {
        ...
        public static Level GenerateFlatgrass(int width, int height, int length)
        {
            Vector3S temp = new Vector3S((ushort)width, (ushort)height, (ushort)length);
            Level map = new Level(temp);
            for (int i = map.Size.y / 2; i >= 0; i--)
                map.FillPlaneXZ(i, Block.BlockList.DIRT);
            map.FillPlaneXZ(map.Size.y / 2, Block.BlockList.GRASS);
            return map;
        }

        
        public static Level GenerateEmpty(int width, int length, int height)
        {
            Vector3S temp = new Vector3S((ushort)width, (ushort)height, (ushort)length);
            return new Level(temp);
        }

        
        public static Level GenerateOcean(int width, int length, int height)
        {
            Vector3S temp = new Vector3S((ushort)width, (ushort)height, (ushort)length);
            Level map = new Level(temp);
            map.Data.MemSet((byte)Block.BlockList.SAND, 0, width * length);
            map.Data.MemSet((byte)Block.BlockList.WATER, width * length, width * length * (height / 2 - 1));
            return map;
        }
        ...
        #region Size.Heightmap Processing

        void GenerateHeightmap()
        {
            ReportProgress(10, "Size.Heightmap: Priming");
            ...
            {
                ReportProgress(2, "Size.Heightmap: Biasing");
            ...
            {
                ReportProgress(10, "Size.Heightmap: Layering");
                ...
                int blendmapDetailSize = (int)Math.Log(Math.Max((double)args.MapWidth, (double)args.MapLength), 2) - 2;
            ...
            {
                ReportProgress(1, "Size.Heightmap: Marbling");
                Noise.Marble(heightmap);
            }

            if (args.InvertHeightmap)
            {
                ReportProgress(1, "Size.Heightmap: Inverting");
        ...
        {
            Vector3S temp = new Vector3S((ushort)args.MapWidth, (ushort)args.MapHeight, (ushort)args.MapLength);
            Level map = new Level(temp);
            ...
            {
                ReportProgress(2, "Size.Heightmap Processing: Matching water coverage");
            ...
            {
                ReportProgress(5, "Size.Heightmap Processing: Adjusting slope");
            ...
            {
                ReportProgress(2, "Size.Heightmap Processing: Smoothing");
            ...
            {
                ReportProgress(5, "Size.Heightmap Processing: Randomizing");
                altmap = new float[map.Size.Width, map.Size.Length];
                int blendmapDetailSize = (int)Math.Log((double)Math.Max(args.MapWidth, args.MapLength), 2) - 2;
                ...
                for (int y = heightmap.GetLength(1) - 1; y >= 0; y--)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (slope < args.CliffThreshold)
                            {
                                map.SetBlock(x, y, level, (byte)(snow ? 36 : bGroundSurface));
                            }
                            else
                            {
                                map.SetBlock(x, y, level, bCliff);
                            ...
                            if (level - i < groundThickness)
                            {
                                ...
                                else
                                {
                                    if (slope < args.CliffThreshold)
                                    {
                                        if (snow)
                                        {
                                            map.SetBlock(x, y, i, Block.BlockList.WHITE_CLOTH);
                                        }
                                        else
                                        {
                                            map.SetBlock(x, y, i, bGround);
            ...
            if (args.AddTrees)
            {
                ...
                {
                    Vector3S temp1 = new Vector3S(map.Size.Width, map.Size.Height, map.Size.Length);
                    Level outLevel = new Level(temp1) { Data = (byte[])map.Data.Clone() };
                    var foresterArgs = new ForesterArgs
                    {
                        Map = map,
                        Rand = rand,
                        TreeCount = (int)(map.Size.Width * map.Size.Length * 4 / (1024f * (args.TreeSpacingMax + args.TreeSpacingMin) / 2)),
                        Operation = Forester.ForesterOperation.Add,
                        PlantOn = bGroundSurface
                    };
                    foresterArgs.BlockPlacing += (sender, e) => outLevel.SetBlock(new Vector3S((ushort)e.Coordinate.X, (ushort)e.Coordinate.Z, (ushort)e.Coordinate.Y), Block.NameToBlock(e.Block.Name));
        ...
        static void AddSingleCave(Random rand, Level map, byte bedrockType, byte fillingType, int length, double maxDiameter)
        {
            ...
            int startX = rand.Next(0, map.Size.Width);
            int startY = rand.Next(0, map.Size.Length);
            int startZ = rand.Next(0, map.Size.Height);

            int k1;
            for (k1 = 0; map.Data[startX + map.Size.Width * map.Size.Length * (map.Size.Height - 1 - startZ) + map.Size.Width * startY] != bedrockType && k1 < 10000; k1++)
            {
                startX = rand.Next(0, map.Size.Width);
                startY = rand.Next(0, map.Size.Length);
                startZ = rand.Next(0, map.Size.Height);
            }
            ...
            {
                int diameter = (int)(maxDiameter * rand.NextDouble() * map.Size.Width);
                    ...
                    for (int k3 = 0; k3 < diameter; k3++)
                    {
                        for (int l3 = 0; l3 < diameter; l3++)
                        {
                            if ((j3 - radius) * (j3 - radius) + (k3 - radius) * (k3 - radius) + (l3 - radius) * (l3 - radius) >= radius * radius ||
                                x + j3 >= map.Size.Width || z + k3 >= map.Size.Height || y + l3 >= map.Size.Length ||
                                x + j3 < 0 || z + k3 < 0 || y + l3 < 0)
                            {
                                continue;
                            }

                            int index = x + j3 + map.Size.Width * map.Size.Length * (map.Size.Height - 1 - (z + k3)) + map.Size.Width * (y + l3);

                            if (map.Data[index] == bedrockType)
                            {
                                map.Data[index] = fillingType;
                            }
        ...
        static void AddSingleVein(Random rand, Level map, byte bedrockType, byte fillingType, int k, double maxDiameter, int l, int i1)
        {
            ...
            int j1 = rand.Next(0, map.Size.Width);
            int k1 = rand.Next(0, map.Size.Height);
            int l1 = rand.Next(0, map.Size.Length);

            double thirteenOverK = 1 / (double)k;

            for (int i2 = 0; i2 < i1; i2++)
            {
                int j2 = j1 + (int)(.5 * (rand.NextDouble() - .5) * map.Size.Width);
                int k2 = k1 + (int)(.5 * (rand.NextDouble() - .5) * map.Size.Height);
                int l2 = l1 + (int)(.5 * (rand.NextDouble() - .5) * map.Size.Length);
                        ...
                        for (int l4 = 0; l4 < diameter; l4++)
                        {
                            for (int i5 = 0; i5 < diameter; i5++)
                            {
                                if ((k4 - radius) * (k4 - radius) + (l4 - radius) * (l4 - radius) + (i5 - radius) * (i5 - radius) < radius * radius &&
                                    i3 + k4 < map.Size.Width && j3 + l4 < map.Size.Height && k3 + i5 < map.Size.Length &&
                                    i3 + k4 >= 0 && j3 + l4 >= 0 && k3 + i5 >= 0)
                                {

                                    int index = i3 + k4 + map.Size.Width * map.Size.Length * (map.Size.Height - 1 - (j3 + l4)) + map.Size.Width * (k3 + i5);
        ...
        {
            for (int x = 1; x < map.Size.Width - 1; x++)
            {
                for (int z = 1; z < map.Size.Height; z++)
                {
                    for (int y = 1; y < map.Size.Length - 1; y++)
                    {
                        int index = map.Index(x, y, z);
                        if ((map.Data[index] == 10 || map.Data[index] == 11 || map.Data[index] == 8 || map.Data[index] == 9) &&
                            (map.GetBlock(x - 1, y, z) == Block.BlockList.AIR || map.GetBlock(x + 1, y, z) == Block.BlockList.AIR ||
                            map.GetBlock(x, y - 1, z) == Block.BlockList.AIR || map.GetBlock(x, y + 1, z) == Block.BlockList.AIR ||
                            map.GetBlock(x, y, z - 1) == Block.BlockList.AIR))
                        {
                            map.Data[index] = sealantType;
                        }
                    }
                }
            }
        ...
        public void AddCaves(Level map)
        {
            if (args.AddCaves)
            {
                ...
                for (int i1 = 0; i1 < 36 * args.CaveDensity; i1++)
                    AddSingleCave(rand, map, (byte)bBedrock, (byte)Block.BlockList.AIR, 30, 0.05 * args.CaveSize);

                for (int j1 = 0; j1 < 9 * args.CaveDensity; j1++)
                    AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.AIR, 500, 0.015 * args.CaveSize, 1);

                for (int k1 = 0; k1 < 30 * args.CaveDensity; k1++)
                    AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.AIR, 300, 0.03 * args.CaveSize, 1, 20);
                ...
                if (args.AddCaveLava)
                {
                    ...
                    {
                        AddSingleCave(rand, map, (byte)bBedrock, (byte)Block.BlockList.LAVA, 30, 0.05 * args.CaveSize);
                    }
                    for (int j = 0; j < 3 * args.CaveDensity; j++)
                    {
                        AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.LAVA, 1000, 0.015 * args.CaveSize, 1);
                    }
                }
                ...
                if (args.AddCaveWater)
                {
                    ...
                    {
                        AddSingleCave(rand, map, (byte)bBedrock, (byte)Block.BlockList.WATER, 30, 0.05 * args.CaveSize);
                    }
                    for (int l = 0; l < 3 * args.CaveDensity; l++)
                    {
                        AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.WATER, 1000, 0.015 * args.CaveSize, 1);
                    }
                }
            ...
            if (args.AddOre)
            {
                ...
                {
                    AddSingleCave(rand, map, (byte)bBedrock, (byte)Block.BlockList.COAL_ORE, 500, 0.03);
                }

                for (int i2 = 0; i2 < 32 * args.CaveDensity; i2++)
                {
                    AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.COAL_ORE, 200, 0.015, 1);
                    AddSingleCave(rand, map, (byte)bBedrock, (byte)Block.BlockList.IRON_ORE, 500, 0.02);
                }

                for (int k2 = 0; k2 < 8 * args.CaveDensity; k2++)
                {
                    AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.IRON_ORE, 200, 0.015, 1);
                    AddSingleVein(rand, map, (byte)bBedrock, (byte)Block.BlockList.GOLD_ORE, 200, 0.0145, 1);
                }

                for (int l2 = 0; l2 < 20 * args.CaveDensity; l2++)
                {
                    AddSingleCave(rand, map, (byte)bBedrock, (byte)Block.BlockList.GOLD_ORE, 400, 0.0175);
                }
            }
        }
        ...
        void AddBeaches( Level map)
        {
            ...
            {
                for (int y = 0; y < map.Size.Length; y++)
                {
                        ...
                        for (int dx = -args.BeachExtent; !found && dx <= args.BeachExtent; dx++)
                        {
                            for (int dy = -args.BeachExtent; !found && dy <= args.BeachExtent; dy++)
                            {
                                for (int dz = -args.BeachHeight; dz <= 0; dz++)
                                {
                                    ...
                                    int yy = y + dy;
                                    int zz = z + dz;
                                    if (xx < 0 || xx >= map.Size.Width || yy < 0 || yy >= map.Size.Length || zz < 0 ||
                                        zz >= map.Size.Height) continue;
        ...
        void GenerateTrees( Level map)
        {
            ...
            for (int x = 0; x < map.Size.Width; x += rn.Next(minTrunkPadding, maxTrunkPadding + 1))
            {
                for (int y = 0; y < map.Size.Length; y += rn.Next(minTrunkPadding, maxTrunkPadding + 1))
                {
                    ...
                    if (nx < 0 || nx >= map.Size.Width || ny < 0 || ny >= map.Size.Length) continue;
                    ...
                    if ((map.GetBlock(nx, ny, nz) == bGroundSurface) && slopemap[nx, ny] < .5)
                    {
                        ...
                        int nh;
                        if ((nh = rn.Next(minHeight, maxHeight + 1)) + nz + nh / 2 > map.Size.Height)
                            continue;

                        // Generate the trunk of the tree
                        for (int z = 1; z <= nh; z++)
                            map.SetBlock(nx, ny, nz + z, Block.BlockList.WOOD);
                        ...
                        for (int i = -1; i < nh / 2; i++)
                        {
                            ...
                            for (int xoff = -radius; xoff < radius + 1; xoff++)
                            {
                                for (int yoff = -radius; yoff < radius + 1; yoff++)
                                {
                                    ...
                                    if (map.GetBlock(nx + xoff, ny + yoff, nz + nh + i) == Block.BlockList.AIR)
                                        map.SetBlock(nx + xoff, ny + yoff, nz + nh + i, Block.BlockList.LEAVES);
                                }
                            }
                        }
                    }
        ...
        void ApplyTheme(MapGenTheme theme)
        {
            ...
            switch (theme)
            {
                case MapGenTheme.Arctic:
                    bWaterSurface = Block.BlockList.GLASS;
                    bDeepWaterSurface = Block.BlockList.WATER;
                    bGroundSurface = Block.BlockList.WHITE_CLOTH;
                    bWater = Block.BlockList.WATER;
                    bGround = Block.BlockList.WHITE_CLOTH;
                    bSeaFloor = Block.BlockList.WHITE_CLOTH;
                    bBedrock = Block.BlockList.STONE;
                    bCliff = Block.BlockList.STONE;
                    groundThickness = 1;
                    break;
                case MapGenTheme.Desert:
                    bWaterSurface = Block.BlockList.WATER;
                    bDeepWaterSurface = Block.BlockList.WATER;
                    bGroundSurface = Block.BlockList.SAND;
                    bWater = Block.BlockList.WATER;
                    bGround = Block.BlockList.SAND;
                    bSeaFloor = Block.BlockList.SAND;
                    bBedrock = Block.BlockList.STONE;
                    bCliff = Block.BlockList.GRAVEL;
                    break;
                case MapGenTheme.Hell:
                    bWaterSurface = Block.BlockList.LAVA;
                    bDeepWaterSurface = Block.BlockList.LAVA;
                    bGroundSurface = Block.BlockList.OBSIDIAN;
                    bWater = Block.BlockList.LAVA;
                    bGround = Block.BlockList.STONE;
                    bSeaFloor = Block.BlockList.OBSIDIAN;
                    bBedrock = Block.BlockList.STONE;
                    bCliff = Block.BlockList.STONE;
                    break;
                case MapGenTheme.Forest:
                    bWaterSurface = Block.BlockList.WATER;
                    bDeepWaterSurface = Block.BlockList.WATER;
                    bGroundSurface = Block.BlockList.GRASS;
                    bWater = Block.BlockList.WATER;
                    bGround = Block.BlockList.DIRT;
                    bSeaFloor = Block.BlockList.SAND;
                    bBedrock = Block.BlockList.STONE;
                    bCliff = Block.BlockList.STONE;
                    break;
                case MapGenTheme.Swamp:
                    bWaterSurface = Block.BlockList.WATER;
                    bDeepWaterSurface = Block.BlockList.WATER;
                    bGroundSurface = Block.BlockList.DIRT;
                    bWater = Block.BlockList.WATER;
                    bGround = Block.BlockList.DIRT;
                    bSeaFloor = Block.BlockList.LEAVES;
                    bBedrock = Block.BlockList.STONE;
                    bCliff = Block.BlockList.STONE;
                    break;
            }
